...
```{r}
#crop plots_buffer
plots_buffer_sb <- st_intersection(plots_buffer, sb_county) %>% 
  dplyr::select(Id, NAME) %>% 
  mutate(Id = 1:8) %>% 
  rename("county" = "NAME")

#new buffer layer with plots_sb
sb_points_buffer <- st_buffer(plots_sb, dist = 100)

leaflet() %>% 
  addPolygons(data = plots_buffer_sb, fillOpacity = 0.75) %>%
  addPolygons(data = sb_points_buffer, color = "red", fillOpacity = 0.25) %>% 
  addTiles()
```
...
## We can also resample raster pixel size, do raster math, and combine raster layers
